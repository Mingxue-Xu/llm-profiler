...
```
## Quick Start
In the root directory of this repo, set `HF_TOKEN` as your own HuggingFace access token (If you don't have HuggingFace access token, create one according to [here](https://huggingface.co/docs/hub/security-tokens)). Then in the terminal,
```shell
...
## Instruction
### Preparation
Give the model name (`MODEL_ID`, which should be available on [HuggingFace](https://huggingface.co/)) and the input batch size `BATCH_SIZE` and maximum sequence length `TEXT_LENGTH`.
```python
...
```
#### 1. Get the total transformer FLOPs (including ADD and MULT) during the inference (a single forwarding pass):
```python
...
transformer with input batch size 16 and sequence length 200:
    MULT:       59.02  (10^9)
```
#### 3. Get the element-wise multiplication operations involved in the *all attention layers*
```python       
...
#### 4. Get the model parameters involved in the *all attention layers*
```python   
...
transformer with input batchsize 16 and sequence length 200:
    ACT:        15.58  (10^9)
```
#### 6. Get the activations (intermediate output between the layers) of the *all the mlp layers*
```python   
...
### What are (not) considered in `llm-profiler`?
...
- `CACHE`: temporary buffers of the layers or the transformer  
...
**NOTE**: For detailed reference code/explanation for FLOPs calculation, a sample reference source code is in [docs/flops](docs/flops.md).
