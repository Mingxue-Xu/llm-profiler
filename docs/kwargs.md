...
Arithmetic operations and model parameters are only determined on the LLM architecture rather than on what kind of devices the LLM is deployed on. 
For this case, the model configuration is enough for the profiler.
...
The default model input is a 2-dimensional tensor, which can be assigned a tensor shape. The first dimension of the tensor is batch size, and the second is the sequence length (or the maximum text length of the input batch). For example,
```python
kwargs = {
    ...
    'input_shape': [16,100],
...
```
means the input tensor has a batch size of 16, and the maximum text length is 100.

#   What Kind of Profiling Data is Available in `llm-profiler`
...
The supported scope in `llm-profiler` is as follows:
1. model-level 
2. layer-level
